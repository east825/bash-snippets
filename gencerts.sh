...
./gencert.sh --signed-by ca.crt ca.key --OU "$TRUSTED_UNIT" \
    --CN "${TRUSTED_SUBDOMAIN}.${CA_DOMAIN}" "$TRUSTED_SUBDOMAIN"

echo ">> Generating expired certificate signed by CA..."
./gencert.sh --signed-by ca.crt ca.key --OU "$EXPIRED_UNIT" \
...
./gencert.sh --signed-by ca.crt ca.key --OU "$WRONG_HOSTNAME_UNIT" \
